        ...
        <div class="col-md-4">
            <div class="border border-subtle rounded mb-4 text-center p-2">
                <div style="width:330px;">
                    <img src="{{ $laborers->image ? asset('storage/' . $laborers->image) : asset('image/profile.jpeg') }}" alt="Receipt Photo" style="width:100%;">
                </div>
                <div>
            ...
            <div class="d-flex justify-content-between gap-3">
                @if (empty($laborers->email_verified_at))
                    <form action="{{ route('verification.resend') }}" method="POST">
                        @csrf
                        <button type="submit" class="btn btn-primary px-5">Resend Verification</button>
                    </form>
                    @else
                    <div></div>
                @endif
                ...
                @if (!empty($laborers->email_verified_at))
                    ...
                    @else
                    <div class="px-3 fs-6">Verify your Email to reset password</div>
                @endif
        ...
        <div class="col-md-8">
            <div class="border rounded mb-3 p-2">
                <div class="row">
                    <div class="form-group mb-3 col-md-4">
                        ...
                        <input type="text" id="name" name="name" class="form-control" value="{{ $laborers->lname }}" readonly>
                    </div>
                    
                    <div class="form-group mb-3 col-md-4">
                        <label for="name" class="form-label">First Name:</label>
                        <input type="text" id="name" name="name" class="form-control" value="{{ $laborers->fname }}" readonly>
                    </div>
                    
                    <div class="form-group col-md-4">
                        <label for="name" class="form-label">Middle Name:</label>
                        <input type="text" id="name" name="name" class="form-control" value="{{ $laborers->mname }}" readonly>
                    </div>
                </div>
            </div>
            ...
            <div class="border p-2 rounded mb-3">
                ...
                <div class="form-group mb-4">
                    ...
                    <textarea id="address" name="address" class="form-control" rows="4" style="width:100%;resize:none;" readonly>{{ trim($laborers->address) }}</textarea>
                    @if (!empty($laborers->email_verified_at))
                    <button class="text-primary float-end border-0 bg-white" data-toggle="modal" data-target="#openModalAddress">edit</button>
                    @else
                        <span class="float-end text-secondary">verify your account to edit information.</span>
                    @endif
                </div>
            </div>
            ...
            <div class="modal fade" id="openModalEmail" tabindex="-1" role="dialog" aria-labelledby="updateModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <form id="updateForm" action="{{ route('updateEmail', $laborers->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="modal-body">
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-sm-8">                                        
                                            <input type="email" class="form-control" id="email" name="email" value="{{ old('email', $laborers->email) }}">
                                            @error('email')
                ...
                <div class="modal fade" id="openModalContact" tabindex="-1" role="dialog" aria-labelledby="updateModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <form id="updateForm" action="{{ route('updateContact', $laborers->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="modal-body">
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" id="contact" name="contact" value="{{ old('contact', $laborers->contact) }}">
                                            @error('contact')
                                                    <div class="text-danger px-2">{{ $message }}</div>
                                            @enderror
                                        </div>
            ...
            <div class="modal fade" id="openModalAddress" tabindex="-1" aria-labelledby="editModalAdress" aria-hidden="true">
                    <div class="modal-dialog modal-lg modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('updateBA', $laborers->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="modal-body">
                                    <div class="form-group mb-3">
                                        <label for="birthdate" class="form-label">Birthday:</label>
                                        <input type="date" id="birthdate" name="birthdate" class="form-control" value="{{ old('birthdate', $laborers->birthdate) }}">
                                        @error('birthdate')
                                                <div class="text-danger px-2">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="row mb-3 px-3">
                                        <textarea id="address" name="address" class="form-control" rows="4" style="resize:none;">{{ old('address', $laborers->address) }}</textarea>
                                        @error('address')
                                                <div class="text-danger px-2">{{ $message }}</div>
                                            @enderror
                                    </div>
