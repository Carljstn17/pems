...
@section('content')
        ...
        <div class="col-md-4">
            ...
            <div class="d-flex justify-content-between mb-4">
                @if (empty($user->email_verified_at))
                    <form action="{{ route('verification.resend.email', ['userId' => $user->id]) }}" method="POST">
                        @csrf
                ...
                @else
                    <div class="text-danger float-end">verify your own email to reset password.</div>
                @endif
            </div>
        </div>
            ...
            <div class="container">
                ...
                <div class="row">
                    ...
                    <div class="form-group col-md-4">
                        ...
                        @else
                            <div class="text-danger float-end">verify your own email to edit information.</div>
                        @endif
                    </div>
                </div>
                </div>
                ...
                <div class="d-flex justify-content-end mb-3">
                    @if (auth()->check() && !empty(auth()->user()->email_verified_at))
                        @if ($user->role === 'staff')
                            @if (!empty($user->srole))
                                <button type="submit" class="btn btn-danger" data-toggle="modal" data-target="#roleModal">Revoke Role</button>
                            @else
                            <!-- Button to assign role -->
                                <button type="submit" class="btn btn-success" data-toggle="modal" data-target="#roleModal">Assign Role</button>
                            @endif
                        @endif
                        @else
                            <div class="text-danger float-end">verify your own email to give special role.</div>
                    @endif
                </div>
                ...
                <div class="modal fade" id="confirmResetModal" tabindex="-1" role="dialog" aria-labelledby="confirmResetModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                Are you sure you want to reset the password to the user's birthday?
                                <br><br>
                                <div>Format: Day Month Year</div>
                                <div>Example: 17092002</div>
                                <br>
                                <div>If user don't have birthday data, the password is 'password'.</div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                                <form id="resetPasswordForm" action="{{ route('reset.password', ['userId' => $user->id]) }}" method="POST">
                                    @csrf
    ...
    <script>
        ...
        $('#confirmResetModal').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget); // Button that triggered the modal
            var modal = $(this);
            
            // Update form action with user ID
            var userId = button.data('userid');
            var form = modal.find('#resetPasswordForm');
            form.attr('action', form.attr('action').replace('userId', userId));
    });
    </script>
